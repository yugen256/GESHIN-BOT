...
bind_count = 1
place_channels = {}  # サーバーごとの設定チャンネル {guild_id: channel_id}

# サーバーごとの履歴を管理する辞書
# {guild_id: {"used_enemies": {...}, "used_restrictions": [...]} }
used_data = {}
...
def get_available_enemy_types(guild_id, area_boss_enabled):
    """サーバーごとに使用可能な敵の種類を取得"""
    guild_enemies = used_data[guild_id]["used_enemies"]
    available = []
    if area_boss_enabled and len(guild_enemies["エリアボス"]) < len(aBosses):
        available.append(("エリアボス", aBosses))
    if len(guild_enemies["週ボス"]) < len(wBosses):
        available.append(("週ボス", wBosses))
    if len(guild_enemies["地方伝説"]) < len(nChars):
        available.append(("地方伝説", nChars))
    return available

def reset_guild_data(guild_id):
    """指定サーバーの履歴をリセット"""
    if guild_id in used_data:
...
async def on_ready():
    ...
    for guild in bot.guilds:
        print(f"🟢 サーバー参加中: {guild.name} ({guild.id})")
        init_guild_data(guild.id)


@bot.event
async def on_message(message):
    global area_boss_enabled, all_boss_enabled, bind_count, place_channels

    if message.author == bot.user or not message.guild:
        return

    guild_id = message.guild.id
    if guild_id not in used_data:
        init_guild_data(guild_id)
    ...
    # ===== /PLACE コマンド =====
    if content == "/PLACE":
        place_channels[guild_id] = message.channel.id
        await message.channel.send("📍 このチャンネルをBotの動作チャンネルに設定しました！")
        return

    # ===== Botが動作するチャンネル以外は無視 =====
    if guild_id not in place_channels or message.channel.id != place_channels[guild_id]:
        return
    ...
    if content.startswith("/AREABOSS "):
        ...
        reset_guild_data(guild_id)
        await message.channel.send("🔄 **このサーバーの履歴をリセットしました！**")

    elif content == "/ENTER":
        result = []

        guild_info = used_data[guild_id]
        used_restrictions = guild_info["used_restrictions"]
        used_enemies = guild_info["used_enemies"]
        ...
        # ===== 敵抽選 =====
        if all_boss_enabled:
            available_types = get_available_enemy_types(guild_id, area_boss_enabled)
